...
import {
  User, Camera, Palette, Bell, Shield, Globe, Settings as SettingsIcon,
  Upload, Trash2, Eye, EyeOff, Smartphone, Mail, Lock, Key,
  Monitor, Sun, Moon, Download, AlertTriangle, Save, X, Check,
  Clock, DollarSign, Calendar, LogOut, Trash, RefreshCw
} from 'lucide-react'
      ...
      content: (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.profilePicture')}</h3>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.personalInfo')}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('settings.jobTitle')}</label>
                <input
                  type="text"
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
          {/* Color Scheme */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.colorScheme')}</h3>
          ...
          {/* Custom Background */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.customBackground')}</h3>
            
            {/* Default Backgrounds */}
            <div className="mb-6">
              <h4 className="text-sm font-medium text-gray-700 mb-3">{t('settings.defaultBackgrounds')}</h4>
                ...
                {defaultBackgrounds.map((bg, index) => (
                  <button
                    ...
                    onClick={async () => {
                      applyBackground(bg)
                      try {
                        await axios.put('/api/user/profile', { custom_background: bg })
                    ...
                    className={`w-full h-16 rounded-lg border-2 transition-colors ${
                      customBackground === bg ? 'border-blue-500' : 'border-gray-200'
                    }`}
                    style={{ background: bg }}
                  />
                ))}
            ...
            <div>
              <h4 className="text-sm font-medium text-gray-700 mb-3">{t('settings.uploadBackground')}</h4>
              <div className="flex items-center space-x-4">
                ...
                {customBackground && !defaultBackgrounds.includes(customBackground) && (
                  <div className="flex items-center space-x-2">
                    <div
                      className="w-12 h-12 rounded-lg border border-gray-200"
                      style={{ backgroundImage: `url(${customBackground})`, backgroundSize: 'cover' }}
                    />
                    <button
                      onClick={async () => {
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {[
                ...
                { key: 'push', label: t('settings.pushNotifications'), icon: Bell },
                { key: 'sms', label: t('settings.smsNotifications'), icon: Smartphone },
                { key: 'marketing', label: t('settings.marketingEmails'), icon: Mail },
                { key: 'security', label: t('settings.securityAlerts'), icon: Shield },
                { key: 'updates', label: t('settings.systemUpdates'), icon: RefreshCw }
              ].map((item) => (
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    {privacy.showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                  </button>
                </div>
              </div>
              <div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.twoFactorAuth')}</h3>
            <div className="flex items-center justify-between">
              ...
              >
                {privacy.twoFactorEnabled ? t('settings.disable2FA') : t('settings.enable2FA')}
              </button>
            </div>
          </div>
        </div>
