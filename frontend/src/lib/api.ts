...
const generateChatResponse = async (message: string, mode: 'serious' | 'joking' = 'joking') => {
  ...
  try {
    ...
    const ollamaResponse = await generateOllamaResponse(message, mode, businessContext, messageAnalysis, timeContext)
    if (ollamaResponse) {
    ...
    console.log('🔄 Ollama unavailable, trying local LLM alternatives...', error.message)
  }
  
  // PRIORITY 2: Try other local LLMs (LM Studio, etc.)
  try {
    console.log('💻 Attempting alternative local LLMs...')
    const localLLMResponse = await generateLocalLLMResponse(message, mode, businessContext, messageAnalysis, timeContext)
    if (localLLMResponse) {
    ...
    console.log('🔄 Local LLMs unavailable, using advanced fallback...', error.message)
  }
  
  // PRIORITY 3: Advanced Intelligent Fallback (Still highly intelligent)
  console.log('🧠 Using advanced intelligent fallback system...')
  return await createAdvancedIntelligentResponse(message, mode, businessContext, messageAnalysis, timeContext)
}

// 🦙 OLLAMA/LLAMA 3.2 INTEGRATION - PREMIUM AI RESPONSES
const generateOllamaResponse = async (message: string, mode: string, businessContext: any, messageAnalysis: any, timeContext: any) => {
...
MESSAGE ANALYSIS:
- Intent: ${messageAnalysis.isGreeting ? 'Greeting' : messageAnalysis.isQuestion ? 'Question' : messageAnalysis.isRequest ? 'Request' : 'Conversational'}
- Topics: ${messageAnalysis.aboutBusiness ? 'Business' : ''}${messageAnalysis.aboutCustomers ? ' Customers' : ''}${messageAnalysis.aboutRevenue ? ' Revenue' : ''}${messageAnalysis.aboutCampaigns ? ' Campaigns' : ''}
...
Provide a comprehensive, intelligent response that addresses their specific needs while incorporating relevant business data. Be specific, actionable, and insightful.`
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (aiResponse && aiResponse.length > 50) { // Lower threshold for faster responses
        console.log('✅ Ollama response generated successfully')
        return {
          response: aiResponse,
    ...
    console.log('Primary Ollama model failed, moving to advanced fallback...', error.message)
    throw new Error('Ollama unavailable - using advanced fallback')
  }
}

// 💻 LOCAL LLM INTEGRATION - LM STUDIO, OOBABOOGA, ETC.
const generateLocalLLMResponse = async (message: string, mode: string, businessContext: any, messageAnalysis: any, timeContext: any) => {
  const systemPrompt = mode === 'serious' 
  ...
  const contextData = `
Business Overview:
- ${businessContext.customers} customers generating ¥${businessContext.revenue.toLocaleString()}
- ${businessContext.roi}% campaign ROI across ${businessContext.activeCampaigns} active campaigns  
- ${businessContext.engagement}% customer engagement rate
- ${businessContext.segments} customer segments identified

Context: ${timeContext.greeting}, ${timeContext.dayOfWeek} ${timeContext.timeOfDay}
User Message: "${message}"
    ...
    const timeoutId = setTimeout(() => controller.abort(), 3000) // Reduced to 3 seconds
      ...
      if (response.ok) {
        ...
        if (aiResponse && aiResponse.length > 50) {
          return {
            response: aiResponse,
            suggestions: generateContextualSuggestions(message, mode, { customerStats: businessContext }),
            ai_engine: endpoint.name
          }
        }
      }
  } catch (error) {
    console.log(`${endpoint.name} failed, using advanced fallback...`, error.message)
    throw new Error('Local LLM endpoints unavailable')
  }
}

// 🧠 ADVANCED INTELLIGENT FALLBACK - HIGHLY SOPHISTICATED
const createAdvancedIntelligentResponse = async (message: string, mode: string, businessContext: any, messageAnalysis: any, timeContext: any) => {
  console.log('🎯 Advanced intelligence system activated - generating highly sophisticated response')
  ...
  const insights = generateAdvancedBusinessInsights(businessContext, messageAnalysis)
  const recommendations = generateStrategicRecommendations(businessContext, messageAnalysis, message)
  const marketContext = generateMarketContextAnalysis(businessContext)
  
  let response = ''
  
  if (messageAnalysis.isGreeting) {
    response = generateAdvancedGreeting(timeContext, businessContext, insights, mode)
  } else if (messageAnalysis.isQuestion) {
    response = generateAdvancedQuestionResponse(message, businessContext, insights, recommendations, mode)
  } else if (messageAnalysis.isRequest) {
    response = generateAdvancedRequestResponse(message, businessContext, recommendations, marketContext, mode)
  } else {
    response = generateAdvancedConversationalResponse(message, businessContext, insights, marketContext, mode)
  }
  
  const contextualSuggestions = generateAdvancedSuggestions(messageAnalysis, businessContext, recommendations)
...
const generateAdvancedBusinessInsights = (businessContext: any, messageAnalysis: any) => {
  const insights = []
  
  // Performance Analysis
  const revenuePerCustomer = Math.round(businessContext.revenue / businessContext.customers)
  const expectedMonthlyGrowth = Math.round(businessContext.customers * 0.12) // Estimate 12% monthly growth
  const marketPenetration = businessContext.engagement > 70 ? 'Strong' : businessContext.engagement > 50 ? 'Moderate' : 'Developing'
  
  insights.push(`Revenue efficiency at ¥${revenuePerCustomer.toLocaleString()} per customer indicates ${revenuePerCustomer > 75000 ? 'premium' : revenuePerCustomer > 50000 ? 'strong' : 'developing'} value realization`)
  
  if (businessContext.roi > 150) {
    insights.push(`Exceptional ${businessContext.roi}% ROI performance suggests highly optimized campaign targeting and premium market positioning`)
  } else if (businessContext.roi > 100) {
    insights.push(`Strong ${businessContext.roi}% ROI demonstrates effective campaign management with room for premium strategy implementation`)
  }
  
  insights.push(`${marketPenetration} market penetration with ${businessContext.engagement}% engagement creates foundation for ${expectedMonthlyGrowth} potential new customers monthly`)
  
  // Segment Analysis
  if (businessContext.segments > 5) {
    insights.push(`Sophisticated ${businessContext.segments}-segment customer architecture enables hyper-personalized targeting and premium value extraction`)
  }
...
const generateStrategicRecommendations = (businessContext: any, messageAnalysis: any, message: string) => {
  ...
  if (lowerMessage.includes('customer') || messageAnalysis.aboutCustomers) {
    recommendations.push({
      ...
      action: `Develop VIP tier program targeting top 20% customers to increase average revenue per customer beyond current ¥${Math.round(businessContext.revenue / businessContext.customers).toLocaleString()}`,
      impact: 'High',
      timeframe: '3-6 weeks'
    })
  ...
  if (lowerMessage.includes('revenue') || lowerMessage.includes('growth') || messageAnalysis.aboutRevenue) {
    recommendations.push({
...
const generateMarketContextAnalysis = (businessContext: any) => {
  const analysis = {
    position: businessContext.roi > 150 ? 'Market Leader' : businessContext.roi > 100 ? 'Strong Competitor' : 'Growing Player',
    opportunity: businessContext.engagement > 70 ? 'Premium Expansion' : 'Engagement Optimization',
    competitive_advantage: `${businessContext.roi}% ROI performance with ${businessContext.engagement}% customer engagement`,
    growth_potential: `${Math.round(businessContext.customers * 0.15)} potential monthly customer acquisition based on current trajectory`
  }
  
  analysis.strategic_focus = businessContext.revenue > 1000000 
...
const generateAdvancedGreeting = (timeContext: any, businessContext: any, insights: any, mode: string) => {
  const greeting = `${timeContext.greeting}! ${mode === 'serious' ? 'Ready to dive into strategic business analysis.' : 'Excited to explore your business landscape together!'}`
  
  const contextualOpener = mode === 'serious' 
    ? `Your SBM operation shows strong fundamentals: ${businessContext.customers.toLocaleString()} customers generating ¥${businessContext.revenue.toLocaleString()} with ${businessContext.roi}% campaign ROI.`
    : `Your SBM empire is looking fantastic - ${businessContext.customers.toLocaleString()} customers, ¥${businessContext.revenue.toLocaleString()} flowing through the system, and campaigns crushing it at ${businessContext.roi}% ROI!`
  
  const insightHighlight = insights.length > 0 ? insights[0] : `Current ${businessContext.engagement}% engagement rate demonstrates strong customer relationships.`
  
  const actionPrompt = mode === 'serious'
...
const generateAdvancedQuestionResponse = (message: string, businessContext: any, insights: any, recommendations: any, mode: string) => {
  const responseStyle = mode === 'serious' 
    ? 'Based on comprehensive business intelligence analysis:'
    : 'Great question! Let me analyze this through your business data lens:'
  
  const dataPoint = insights.length > 0 ? insights[0] : `Your ${businessContext.customers.toLocaleString()} customers with ${businessContext.engagement}% engagement create strong foundation for strategic initiatives.`
...
const generateAdvancedRequestResponse = (message: string, businessContext: any, recommendations: any, marketContext: any, mode: string) => {
  const lowerMessage = message.toLowerCase()
  
  // Detect specific request types and provide comprehensive responses
  if (lowerMessage.includes('campaign') && (lowerMessage.includes('young') || lowerMessage.includes('adult'))) {
    return generateYoungAdultCampaignStrategy(businessContext, [], mode)
  } else if (lowerMessage.includes('campaign') && (lowerMessage.includes('creat') || lowerMessage.includes('design') || lowerMessage.includes('build'))) {
    return generateCampaignCreationStrategy(businessContext, [], mode)
  } else if (lowerMessage.includes('segment') || (lowerMessage.includes('target') && lowerMessage.includes('audienc'))) {
    return generateSegmentationStrategy(businessContext, [], mode)
  } else if (lowerMessage.includes('customer') && (lowerMessage.includes('acqui') || lowerMessage.includes('retention') || lowerMessage.includes('loyalty'))) {
    return generateCustomerStrategy(businessContext, [], mode)
  } else if (lowerMessage.includes('revenue') || lowerMessage.includes('growth') || lowerMessage.includes('income') || lowerMessage.includes('profit')) {
    return generateRevenueGrowthStrategy(businessContext, [], mode)
  } else if (lowerMessage.includes('analyz') || lowerMessage.includes('report') || lowerMessage.includes('insight')) {
    return generateAnalyticsStrategy(businessContext, [], mode)
  } else if (lowerMessage.includes('optim') || lowerMessage.includes('improv') || lowerMessage.includes('better')) {
    return generateOptimizationStrategy(businessContext, [], mode)
  }
  
  // Default comprehensive request response
  const acknowledgment = mode === 'serious'
  ...
  const contextualInsight = `Given your current ${marketContext.position} position with ¥${businessContext.revenue.toLocaleString()} revenue and ${businessContext.roi}% ROI,`
  
  const strategicDirection = recommendations.length > 0
...
const generateAnalyticsStrategy = (businessContext: any, insights: any, mode: string) => {
  return `${mode === 'serious' ? 'Advanced analytics implementation requires comprehensive data integration, performance measurement, and actionable insight generation.' : 'Love the analytical mindset! Let\'s turn your data into business gold.'}

COMPREHENSIVE ANALYTICS STRATEGY:

📊 CURRENT ANALYTICS FOUNDATION:
Your ${businessContext.customers} customers generating ¥${businessContext.revenue.toLocaleString()} with ${businessContext.roi}% ROI provide rich data foundation for advanced analytics implementation.
...
const generateOptimizationStrategy = (businessContext: any, insights: any, mode: string) => {
  return `${mode === 'serious' ? 'Business optimization requires systematic performance analysis, bottleneck identification, and efficiency enhancement across all operational areas.' : 'Optimization time! Let\'s supercharge your business performance.'}

COMPREHENSIVE OPTIMIZATION STRATEGY:

⚡ CURRENT PERFORMANCE ANALYSIS:
${businessContext.roi}% ROI with ¥${businessContext.revenue.toLocaleString()} revenue shows strong foundation with significant optimization potential across ${businessContext.segments} customer segments.
...
const generateAdvancedConversationalResponse = (message: string, businessContext: any, insights: any, marketContext: any, mode: string) => {
  const lowerMessage = message.toLowerCase()
  
  // Detect specific business tasks and provide comprehensive analysis
  if (lowerMessage.includes('campaign') && (lowerMessage.includes('young') || lowerMessage.includes('adult') || lowerMessage.includes('youth'))) {
    return generateYoungAdultCampaignStrategy(businessContext, insights, mode)
  } else if (lowerMessage.includes('campaign') && lowerMessage.includes('creat')) {
    return generateCampaignCreationStrategy(businessContext, insights, mode)
  } else if (lowerMessage.includes('segment') || lowerMessage.includes('target')) {
    return generateSegmentationStrategy(businessContext, insights, mode)
  } else if (lowerMessage.includes('customer') && (lowerMessage.includes('acqui') || lowerMessage.includes('retention'))) {
    return generateCustomerStrategy(businessContext, insights, mode)
  } else if (lowerMessage.includes('revenue') || lowerMessage.includes('growth')) {
    return generateRevenueGrowthStrategy(businessContext, insights, mode)
  }
  
  // Default comprehensive business analysis
  const engagement = mode === 'serious'
    ? 'I appreciate your perspective on this business matter.'
    : 'That\'s a really interesting angle to explore!'
  
  const businessConnection = `Your current ${marketContext.position} market position with ${businessContext.customers.toLocaleString()} customers provides excellent foundation for strategic growth initiatives.`
  
  const insight = insights.length > 0 ? insights[Math.floor(Math.random() * insights.length)] : `${businessContext.engagement}% engagement rate indicates strong customer relationships with significant expansion potential.`
  
  const forwardLooking = mode === 'serious'
...
const generateYoungAdultCampaignStrategy = (businessContext: any, insights: any, mode: string) => {
  const intro = mode === 'serious' 
...
const generateCampaignCreationStrategy = (businessContext: any, insights: any, mode: string) => {
  return `${mode === 'serious' ? 'Strategic campaign development requires systematic approach integrating market analysis, creative strategy, and performance optimization.' : 'Excellent! Let\'s create a campaign that truly delivers results for your business.'}

COMPREHENSIVE CAMPAIGN CREATION FRAMEWORK:

📊 FOUNDATIONAL ANALYSIS:
Your ${businessContext.customers} customers generating ¥${businessContext.revenue.toLocaleString()} provide strong foundation for campaign expansion. With ${businessContext.roi}% ROI, you're positioned for strategic scaling.

🎯 CAMPAIGN STRATEGY BLUEPRINT:
• Audience Definition: Leverage your ${businessContext.segments} customer segments
...
• Budget Allocation: Strategic investment based on historical ${businessContext.roi}% performance
...
This framework leverages your proven ${businessContext.roi}% ROI to systematically expand market reach while maintaining profitability. Ready to dive into specific campaign elements?`
}

// 📊 SEGMENTATION STRATEGY GENERATOR  
const generateSegmentationStrategy = (businessContext: any, insights: any, mode: string) => {
  return `${mode === 'serious' ? 'Advanced customer segmentation enables precision targeting and maximizes campaign efficiency through data-driven audience analysis.' : 'Smart thinking! Proper segmentation is the secret sauce to marketing success.'}

ADVANCED SEGMENTATION STRATEGY:

🔍 CURRENT SEGMENTATION ANALYSIS:
Your ${businessContext.segments} existing segments with ${businessContext.customers} customers show strong foundation. ${businessContext.engagement}% engagement suggests effective targeting potential.
...
This approach can increase your current ${businessContext.roi}% ROI by 40-70% through precision targeting. Which segments interest you most?`
}

// 👥 CUSTOMER STRATEGY GENERATOR
const generateCustomerStrategy = (businessContext: any, insights: any, mode: string) => {
  return `${mode === 'serious' ? 'Customer lifecycle optimization requires integrated approach combining acquisition efficiency with retention excellence and value maximization.' : 'Perfect focus! Your customers are your greatest asset - let\'s maximize their potential.'}

COMPREHENSIVE CUSTOMER STRATEGY:

👥 CUSTOMER LIFECYCLE ANALYSIS:
Current base of ${businessContext.customers} customers with ${businessContext.engagement}% engagement provides excellent foundation for strategic growth.
...
With your current ¥${(businessContext.revenue / businessContext.customers).toFixed(0)} average customer value, strategic optimization can increase LTV by 50-150%. Ready to explore specific tactics?`
}

// 💰 REVENUE GROWTH STRATEGY GENERATOR
const generateRevenueGrowthStrategy = (businessContext: any, insights: any, mode: string) => {
  return `${mode === 'serious' ? 'Revenue acceleration requires systematic approach integrating market expansion, pricing optimization, and operational efficiency improvements.' : 'Excellent focus on growth! Let\'s unlock your revenue potential.'}

STRATEGIC REVENUE GROWTH FRAMEWORK:

💰 CURRENT REVENUE ANALYSIS:
¥${businessContext.revenue.toLocaleString()} revenue from ${businessContext.customers} customers (¥${(businessContext.revenue / businessContext.customers).toFixed(0)} per customer) with ${businessContext.roi}% ROI shows strong foundation for scaling.
...
Target: 150-300% revenue growth within 12 months, building on your proven ${businessContext.roi}% ROI foundation. Which growth lever interests you most?`
}

// 💡 ADVANCED SUGGESTION GENERATOR
const generateAdvancedSuggestions = (messageAnalysis: any, businessContext: any, recommendations: any) => {
  const suggestions = []
  ...
  if (recommendations.length > 0) {
    ...
    suggestions.unshift(`Implement ${topRec.category.toLowerCase()}: ${topRec.action.split(' ').slice(0, 8).join(' ')}...`)
  }
...
const analyzeMessageDeep = (message: string) => {
  ...
  return {
    // Intent Analysis
    isGreeting: /^(hello|hi|hey|good\s+(morning|afternoon|evening)|greetings?)/.test(lowerMessage),
    isQuestion: /\?|^(what|how|why|when|where|which|who|can you|could you|would you|is|are|do|does)/.test(lowerMessage),
    isRequest: /(please|help|assist|show|tell|explain|describe|analyze|generate|create|make|build)/.test(lowerMessage),
    
    // Topic Analysis
    aboutBusiness: /(business|company|organization|enterprise)/.test(lowerMessage),
    aboutCustomers: /(customer|client|user|visitor|buyer)/.test(lowerMessage),
    aboutRevenue: /(revenue|money|profit|income|sales|earning)/.test(lowerMessage),
    aboutCampaigns: /(campaign|marketing|promotion|advertising|ad)/.test(lowerMessage),
    aboutAnalytics: /(analytics|data|metrics|stats|performance|insights)/.test(lowerMessage),
    
    // Sentiment Analysis
    sentiment: /(good|great|excellent|amazing|fantastic|wonderful)/.test(lowerMessage) ? 'positive' :
               /(bad|terrible|awful|problem|issue|error|wrong|fail)/.test(lowerMessage) ? 'negative' : 'neutral',
    
    // Context Clues
    needsHelp: /(help|assist|support|guide)/.test(lowerMessage),
    isExploring: /(what|explore|discover|find|learn|understand)/.test(lowerMessage),
...
const getTimeContext = () => {
  ...
  return {
    timeOfDay: hour < 6 ? 'early morning' : hour < 12 ? 'morning' : hour < 17 ? 'afternoon' : hour < 20 ? 'evening' : 'night',
...
const generateUniqueResponse = (message: string, mode: string, analysis: any, timeContext: any, businessContext: any) => {
  if (analysis.isGreeting) {
    return generateGreetingResponse(message, mode, timeContext, businessContext)
    ...
    return generateConversationalResponse(message, mode, analysis, timeContext, businessContext)
  }
}

// 👋 GREETING RESPONSE GENERATOR
const generateGreetingResponse = (message: string, mode: string, timeContext: any, businessContext: any) => {
  ...
  const businessInsights = [
    ...
    `The numbers are amazing - ${businessContext.customers.toLocaleString()} customers engaged at ${businessContext.engagement}% with stellar ${businessContext.roi}% returns!`
  ]
  
  const excitement = mode === 'serious' ? [
  ...
  const greeting = greetingStyles[Math.floor(Math.random() * greetingStyles.length)]
  const insight = businessInsights[Math.floor(Math.random() * businessInsights.length)]
...
const generateQuestionResponse = (message: string, mode: string, analysis: any, businessContext: any) => {
  const questionStarters = mode === 'serious' ? [
  ...
  const insights = [
    `Your current performance metrics show strong foundation with ${businessContext.customers.toLocaleString()} engaged customers and ${businessContext.roi}% campaign ROI.`,
    ...
    `Your business intelligence reveals ${businessContext.activeCampaigns} active initiatives generating ¥${businessContext.revenue.toLocaleString()} in revenue.`,
    `Analytics demonstrate solid performance with diversified customer base and optimized campaign effectiveness.`
  ]
  
  const actionables = mode === 'serious' ? [
  ...
  const actionable = actionables[Math.floor(Math.random() * actionables.length)]
  
  return `${starter} ${insight} ${actionable} Would you like me to elaborate on any specific aspect?`
}

// 🎯 REQUEST RESPONSE GENERATOR
const generateRequestResponse = (message: string, mode: string, analysis: any, businessContext: any) => {
  const acknowledgments = mode === 'serious' ? [
  ...
  const context = `Given your current business position with ${businessContext.customers.toLocaleString()} customers, ¥${businessContext.revenue.toLocaleString()} revenue, and ${businessContext.roi}% ROI performance,`
...
const generateConversationalResponse = (message: string, mode: string, analysis: any, timeContext: any, businessContext: any) => {
  const conversationStarters = mode === 'serious' ? [
  ...
  const transitions = mode === 'serious' ? [
  ...
  const insights = [
    `your ${businessContext.customers.toLocaleString()} customers are generating impressive ${businessContext.engagement}% engagement rates with solid ${businessContext.roi}% returns.`,
...
const generateDynamicSuggestions = (analysis: any, mode: string) => {
  ...
  if (analysis.aboutCustomers) {
    contextualSuggestions.push('Deep dive into customer segmentation insights', 'Analyze customer lifetime value patterns')
  }
  if (analysis.aboutRevenue) {
    contextualSuggestions.push('Explore revenue optimization strategies', 'Identify high-ROI opportunities')
  }
  if (analysis.aboutCampaigns) {
    contextualSuggestions.push('Optimize campaign performance metrics', 'Design targeted audience strategies')
  }
  if (analysis.aboutAnalytics) {
    contextualSuggestions.push('Generate comprehensive business reports', 'Create predictive analytics insights')
  }
...
const generateLLMResponse = async (message: string, mode: string, data: any) => {
  const systemPrompt = mode === 'serious' 
...
const generateContextualSuggestions = (message: string, mode: string, data?: any) => {
  const lowerMessage = message.toLowerCase()
  ...
  discoveredSegments.forEach((segment: any) => {
    ...
    const matchesSegment = segmentKeywords.some(keyword => 
    ...
    if (matchesSegment) {
      ...
      dynamicSegmentContext[`segment_${segment.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}`] = true
    }
  })
  ...
  const context = {
    ...
    isYoungAdults: lowerMessage.includes('young adult') || lowerMessage.includes('18-25') || lowerMessage.includes('millennials'),
    isTeens: lowerMessage.includes('teen') || lowerMessage.includes('teenager') || lowerMessage.includes('young teen'),
    isElderly: lowerMessage.includes('elderly') || lowerMessage.includes('elderlies') || lowerMessage.includes('senior') || lowerMessage.includes('seniors') || lowerMessage.includes('older adult') || lowerMessage.includes('65+') || lowerMessage.includes('retiree') || lowerMessage.includes('mature'),
    isMiddleAged: lowerMessage.includes('middle age') || lowerMessage.includes('35-50') || lowerMessage.includes('working professional') || lowerMessage.includes('parent'),
    
    // Business context detection
    isLocal: lowerMessage.includes('local') || lowerMessage.includes('neighborhood') || lowerMessage.includes('community'),
    isNational: lowerMessage.includes('national') || lowerMessage.includes('domestic') || lowerMessage.includes('country-wide') || lowerMessage.includes('nationwide'),
    isInternational: lowerMessage.includes('international') || lowerMessage.includes('global') || lowerMessage.includes('overseas') || lowerMessage.includes('foreign'),
    isCorporate: lowerMessage.includes('corporate') || lowerMessage.includes('enterprise') || lowerMessage.includes('business') || lowerMessage.includes('company'),
    isHealthcare: lowerMessage.includes('healthcare') || lowerMessage.includes('medical') || lowerMessage.includes('hospital') || lowerMessage.includes('clinic'),
    isEducational: lowerMessage.includes('educational') || lowerMessage.includes('university') || lowerMessage.includes('school') || lowerMessage.includes('academic'),
    isGovernment: lowerMessage.includes('government') || lowerMessage.includes('public sector') || lowerMessage.includes('municipal') || lowerMessage.includes('federal'),
    isStartup: lowerMessage.includes('startup') || lowerMessage.includes('small business') || lowerMessage.includes('emerging company'),
    isB2B: lowerMessage.includes('b2b') || lowerMessage.includes('business to business'),
    isB2C: lowerMessage.includes('b2c') || lowerMessage.includes('business to consumer') || lowerMessage.includes('consumer'),
    isB2G: lowerMessage.includes('b2g') || lowerMessage.includes('business to government'),
    
    // Campaign and platform context
    isCampaign: lowerMessage.includes('campaign') || lowerMessage.includes('marketing') || lowerMessage.includes('strategy') || lowerMessage.includes('how about') || lowerMessage.includes('what about'),
    isCustomize: lowerMessage.includes('customize') || lowerMessage.includes('custom'),
    isSummer: lowerMessage.includes('summer'),
    isWinter: lowerMessage.includes('winter'),
    isShanghai: lowerMessage.includes('shanghai'),
    platform: lowerMessage.includes('tiktok') ? 'TikTok' : lowerMessage.includes('instagram') ? 'Instagram' : lowerMessage.includes('wechat') ? 'WeChat' : null,
  ...
  const matchedSegments = discoveredSegments.filter((segment: any) => 
    dynamicSegmentContext[segment.id] || dynamicSegmentContext[`segment_${segment.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}`]
  )
  ...
  if (context.isCampaign && matchedSegments.length > 0) {
    ...
    const segmentSuggestions = [
      ...
      `Create targeted content for ${segment.name} based on ${segment.patterns.join(', ')}`
    ]
  ...
  if (context.isCampaign && discoveredSegments.length > 0) {
    ...
    return [
      `Campaign strategies for our top segments: ${topSegments.map((s: any) => s.name).join(', ')}`,
      `How to target our ${discoveredSegments.length} discovered customer segments effectively?`,
      `Cross-segment campaign optimization across ${topSegments.map((s: any) => s.name).join(' and ')}`,
      `Personalized messaging for ${topSegments[0]?.name || 'premium'} vs ${topSegments[1]?.name || 'standard'} segments`
    ]
  }
  ...
  if (context.isCampaign && context.isElderly) {
    ...
    const isWinter = lowerMessage.includes('winter') || lowerMessage.includes('cold') || lowerMessage.includes('holiday')
  ...
  if (context.isCampaign) {
    const demographic = context.isYoungAdults ? 'young adults' : 
                       context.isTeens ? 'teens' : 
                       context.isElderly ? 'seniors' :
                       context.isMiddleAged ? 'middle-aged adults' : 'target audience'
    return [
...
const generateFallbackResponse = (message: string, mode: string, data: any) => {
  ...
  const queryAnalysis = {
    isQuestion: lowerMessage.includes('?') || lowerMessage.startsWith('what') || lowerMessage.startsWith('how') || lowerMessage.startsWith('why') || lowerMessage.startsWith('when') || lowerMessage.startsWith('where') || lowerMessage.startsWith('which'),
    isRequest: lowerMessage.includes('generate') || lowerMessage.includes('create') || lowerMessage.includes('plan') || lowerMessage.includes('design') || lowerMessage.includes('customize') || lowerMessage.includes('develop') || lowerMessage.includes('build') || lowerMessage.includes('campaign') || lowerMessage.includes('make') || lowerMessage.includes('help'),
    isAnalysis: lowerMessage.includes('analyze') || lowerMessage.includes('review') || lowerMessage.includes('assess') || lowerMessage.includes('examine') || lowerMessage.includes('evaluate'),
    isCampaign: lowerMessage.includes('campaign') || lowerMessage.includes('marketing') || lowerMessage.includes('strategy') || lowerMessage.includes('promotion') || lowerMessage.includes('elderly') || lowerMessage.includes('elderlies') || lowerMessage.includes('senior') || lowerMessage.includes('teen') || lowerMessage.includes('young') || lowerMessage.includes('winter') || lowerMessage.includes('summer') || lowerMessage.includes('audience') || lowerMessage.includes('target'),
    isGreeting: lowerMessage.includes('hello') || lowerMessage.includes('hi') || lowerMessage.includes('hey') || lowerMessage.startsWith('good'),
    sentiment: lowerMessage.includes('good') || lowerMessage.includes('great') || lowerMessage.includes('excellent') ? 'positive' : 
               lowerMessage.includes('bad') || lowerMessage.includes('problem') || lowerMessage.includes('issue') || lowerMessage.includes('wrong') ? 'negative' : 'neutral',
    urgency: lowerMessage.includes('urgent') || lowerMessage.includes('asap') || lowerMessage.includes('immediately') || lowerMessage.includes('now') ? 'high' : 'normal'
  }
  ...
  } else if ((queryAnalysis.isRequest && queryAnalysis.isCampaign) || queryAnalysis.isCampaign) {
...
const generateIntelligentCampaignResponse = (message: string, mode: string, data: any) => {
  const lowerMessage = message.toLowerCase()
  
  // Analyze the specific campaign request
  const targetAnalysis = {
    // Demographics
    isYoungAdults: lowerMessage.includes('young adult') || lowerMessage.includes('18-25') || lowerMessage.includes('millennials') || lowerMessage.includes('gen z'),
    isTeens: lowerMessage.includes('teen') || lowerMessage.includes('teenager') || lowerMessage.includes('13-19') || lowerMessage.includes('young teen'),
    isElderly: lowerMessage.includes('elderly') || lowerMessage.includes('elderlies') || lowerMessage.includes('senior') || lowerMessage.includes('seniors') || lowerMessage.includes('older adult') || lowerMessage.includes('65+') || lowerMessage.includes('retiree') || lowerMessage.includes('mature'),
    isMiddleAged: lowerMessage.includes('middle age') || lowerMessage.includes('35-50') || lowerMessage.includes('working professional') || lowerMessage.includes('parent'),
    ...
    // Seasonal & Location
    isSummer: lowerMessage.includes('summer'),
    isWinter: lowerMessage.includes('winter') || lowerMessage.includes('cold') || lowerMessage.includes('holiday') || lowerMessage.includes('christmas') || lowerMessage.includes('new year'),
    isShanghai: lowerMessage.includes('shanghai'),
    isCustomize: lowerMessage.includes('customize') || lowerMessage.includes('custom'),
    platform: lowerMessage.includes('tiktok') ? 'TikTok' : lowerMessage.includes('instagram') ? 'Instagram' : lowerMessage.includes('wechat') ? 'WeChat' : lowerMessage.includes('social') ? 'Social Media' : 'Multi-platform'
  }
  
  const currentPerformance = `Current SBM metrics: ${data.customerStats.total} customers, ¥${data.dashboardStats.totalRevenue.toLocaleString()} revenue, ${data.campaignStats.avgROI}% ROI.`
  
  if (mode === 'serious') {
    return generateSeriousCampaignStrategy(message, targetAnalysis, data, currentPerformance)
  } else {
    return generateCreativeCampaignStrategy(message, targetAnalysis, data, currentPerformance)
  }
}

const generateSeriousCampaignStrategy = (message, analysis, data, performance) => {
  ...
  const demographicFocus = matchingSegment ? 
    `${matchingSegment.name.toUpperCase()} (${matchingSegment.count} customers - ${matchingSegment.patterns.join(', ')})` :
    // Fallback to traditional analysis
    analysis.isYoungAdults ? 'YOUNG ADULTS (18-25)' : 
    analysis.isTeens ? 'TEENS (13-19)' : 
    analysis.isElderly ? 'SENIORS (65+)' :
    analysis.isMiddleAged ? 'MIDDLE-AGED ADULTS (35-50)' :
    analysis.isHealthcare ? 'HEALTHCARE SECTOR' :
    analysis.isEducational ? 'EDUCATIONAL INSTITUTIONS' :
    analysis.isGovernment ? 'GOVERNMENT SECTOR' :
    analysis.isStartup ? 'STARTUP COMPANIES' :
    analysis.isCorporate ? 'CORPORATE ENTERPRISES' :
    analysis.isLocal ? 'LOCAL BUSINESSES' :
    analysis.isNational ? 'NATIONAL MARKET' :
    analysis.isInternational ? 'INTERNATIONAL MARKET' :
    analysis.isB2B ? 'B2B CLIENTS' :
    analysis.isB2C ? 'B2C CONSUMERS' :
    analysis.isB2G ? 'B2G SECTOR' :
    analysis.isWinter ? 'WINTER SHOPPERS' : 'TARGET DEMOGRAPHIC'
...
- Expected ROI: ${Math.round(data.campaignStats.avgROI * 1.35)}% (35% improvement)
...
const generateCreativeCampaignStrategy = (message, analysis, data, performance) => {
  ...
  const demographicFocus = matchingSegment ? 
    `${matchingSegment.name.toLowerCase()} (${matchingSegment.count} amazing customers who love ${matchingSegment.patterns.join(' and ')})` :
    // Fallback to traditional analysis
    analysis.isYoungAdults ? 'young adults' : 
    analysis.isTeens ? 'teens' : 
    analysis.isElderly ? 'seniors' :
    analysis.isMiddleAged ? 'middle-aged adults' :
    analysis.isHealthcare ? 'healthcare professionals' :
    analysis.isEducational ? 'educational institutions' :
    analysis.isGovernment ? 'government agencies' :
    analysis.isStartup ? 'startup companies' :
    analysis.isCorporate ? 'corporate clients' :
    analysis.isLocal ? 'local businesses' :
    analysis.isNational ? 'national market' :
    analysis.isInternational ? 'international clients' :
    analysis.isB2B ? 'business clients' :
    analysis.isB2C ? 'consumers' :
    analysis.isB2G ? 'government sector' :
    analysis.isWinter ? 'winter shoppers' : 'your target audience'
  const budget = Math.round(data.dashboardStats.totalRevenue * 0.12)
  ...
  analysis.isHealthcare ? [
    'Healing & Innovation Partnership', 'Medical Excellence Initiative', 'Healthcare Heroes Support', 'Patient Care Revolution',
    'Clinical Excellence Program', 'Wellness Partnership Project', 'Medical Innovation Alliance', 'Healthcare Quality Focus'
  ] : analysis.isEducational ? [
    'Academic Excellence Initiative', 'Learning Innovation Partnership', 'Educational Transformation', 'Future Leaders Program',
    'Knowledge Empowerment Project', 'Academic Success Alliance', 'Educational Excellence Movement', 'Learning Revolution'
  ] : analysis.isGovernment ? [
    'Public Service Excellence', 'Community Impact Initiative', 'Citizen Service Revolution', 'Government Innovation Partnership',
    'Public Sector Modernization', 'Transparency & Efficiency Program', 'Public Value Creation', 'Community Partnership Project'
  ] : analysis.isStartup ? [
    'Innovation Accelerator Program', 'Startup Success Partnership', 'Growth Catalyst Initiative', 'Entrepreneurial Excellence',
    'Future Builders Alliance', 'Innovation Ecosystem Project', 'Startup Empowerment Movement', 'Growth Partnership Program'
  ] : analysis.isInternational ? [
    'Global Partnership Initiative', 'International Excellence Program', 'Cross-Border Innovation', 'Global Market Leadership',
    'International Growth Alliance', 'Global Business Partnership', 'Cross-Cultural Success', 'International Expansion Project'
  ] : analysis.isLocal ? [
    'Community Champions Program', 'Local Pride Initiative', 'Neighborhood Excellence', 'Community Partnership Project',
    'Local Business Empowerment', 'Community Connection Campaign', 'Local Market Leadership', 'Neighborhood Success Story'
  ] : analysis.isNational ? [
    'National Excellence Initiative', 'Nationwide Partnership Program', 'Country-Wide Success', 'National Market Leadership',
    'Cross-Regional Excellence', 'National Growth Alliance', 'Nationwide Innovation Project', 'Country-Wide Partnership'
  ] : analysis.isElderly && analysis.isWinter ? [
    'Winter Golden Years', 'Cozy Heritage Moments', 'Holiday Wisdom Circle', 'Warm Traditions Campaign',
    'Festive Legacy Living', 'Winter Comfort & Care', 'Holiday Heritage Focus', 'Seasonal Golden Moments'
  ] : analysis.isElderly ? [
    'Golden Moments Celebration', 'Wisdom & Warmth Initiative', 'Heritage & Quality Focus', 'Comfort & Care Campaign',
    'Legacy Living Project', 'Premium Service Experience', 'Trust & Tradition Movement', 'Timeless Elegance Journey'
  ] : analysis.isWinter ? [
    'Winter Warmth Campaign', 'Cozy Comfort Initiative', 'Holiday Spirit Movement', 'New Year New You',
    'Winter Wellness Journey', 'Festive Celebration Project', 'Cold Weather Solutions', 'Indoor Comfort Focus'  
  ] : [
    'Authenticity Revolution', 'Digital Nomad Lifestyle', 'Sustainable Future', 'Creative Expression Hub',
    'Tech Innovation Playground', 'Social Impact Movement', 'Cultural Bridge Project', 'Mindful Living Journey'
  ]
  
  const selectedConcept = creativeConcepts[Math.floor(Math.random() * creativeConcepts.length)]
...
In 90 days, SBM won't just be a brand that ${demographicFocus} buy from - it'll be a brand they identify with, recommend to friends, and feel proud to support. Expected revenue impact: ¥${Math.round(budget * 3.2).toLocaleString()}, but more importantly, lasting brand loyalty.
...
const generateIntelligentAnalysisResponse = (message: string, mode: string, data: any) => {
  const keyMetrics = {
    customerGrowth: Math.round((data.customerStats.total / 12) * 100) / 100, // Monthly average
    revenuePerCustomer: Math.round(data.dashboardStats.totalRevenue / data.customerStats.total),
    vipRatio: Math.round((data.customerStats.vip / data.customerStats.total) * 100),
    campaignEfficiency: Math.round(data.campaignStats.avgROI / data.campaignStats.active * 100) / 100
  }

  if (mode === 'serious') {
    return `BUSINESS INTELLIGENCE ANALYSIS\n\nCurrent Performance Metrics:\n- Total Customer Base: ${data.customerStats.total} (Growth rate: ${data.dashboardStats.growthRate}%)\n- Revenue Performance: ¥${data.dashboardStats.totalRevenue.toLocaleString()} (¥${keyMetrics.revenuePerCustomer.toLocaleString()} per customer)\n- Premium Segment: ${data.customerStats.vip} VIP customers (${keyMetrics.vipRatio}% of total base)\n- Campaign Performance: ${data.campaignStats.active} active campaigns, ${data.campaignStats.avgROI}% average ROI\n\nKey Insights:\n1. Customer Acquisition: Current growth trajectory suggests ${Math.round(keyMetrics.customerGrowth)} new customers monthly\n2. Revenue Optimization: VIP customers likely generate 3-4x average customer value\n3. Campaign Efficiency: ROI performance indicates strong market-product fit\n4. Engagement Quality: ${data.customerStats.avgEngagement}% engagement rate shows healthy customer relationships\n\nStrategic Recommendations:\n- Focus on VIP segment expansion (highest ROI potential)\n- Optimize campaign budget allocation based on performance data\n- Implement retention strategies for ${data.customerStats.inactive} inactive customers\n- Scale successful campaign elements across broader audience\n\nRisk Factors: Market saturation, competitive pressure, customer acquisition costs\nOpportunity Areas: Premium service expansion, cross-selling, market diversification`
  } else {
    return `Let me break down what your numbers are really telling us - and trust me, there are some fascinating stories hidden in this data!\n\nYour ${data.customerStats.total} customers are generating ¥${data.dashboardStats.totalRevenue.toLocaleString()} - that's ¥${keyMetrics.revenuePerCustomer.toLocaleString()} per person on average. Not bad at all!\n\nBut here's where it gets interesting: your ${data.customerStats.vip} VIP customers are probably your secret weapon. They represent ${keyMetrics.vipRatio}% of your base but likely drive way more than their fair share of revenue. These are your brand champions, your word-of-mouth generators, your "I'll-pay-premium-for-quality" customers.\n\nYour campaigns are performing at ${data.campaignStats.avgROI}% ROI across ${data.campaignStats.active} active initiatives. That's actually pretty solid - it means you're not just throwing money at the wall and hoping something sticks.\n\nThe ${data.customerStats.avgEngagement}% engagement rate tells me people actually care about what you're putting out there. In today's attention-deficit world, that's like finding a unicorn.\n\nHere's what I'd focus on: those ${data.customerStats.inactive} inactive customers are sleeping goldmines. A good reactivation campaign could wake them up and turn them back into revenue generators.\n\nThe growth rate of ${data.dashboardStats.growthRate}% suggests you're on the right track - now it's about scaling what works and fixing what doesn't!`
  }
}

// Intelligent Question Response
const generateIntelligentQuestionResponse = (message: string, mode: string, data: any) => {
  const lowerMessage = message.toLowerCase()
  ...
  if (lowerMessage.includes('how many') || lowerMessage.includes('what is')) {
    ...
    return mode === 'serious' 
      ? `Based on current data analysis: Total customers: ${numbers.customers}, VIP segment: ${numbers.vip}, Revenue: ¥${numbers.revenue.toLocaleString()}, Active campaigns: ${numbers.campaigns}, Average ROI: ${numbers.roi}%, Customer engagement: ${numbers.engagement}%. These metrics indicate strong business fundamentals with opportunities for optimization in customer acquisition and retention strategies.`
      : `Great question! Let me give you the scoop: You've got ${numbers.customers} customers in your ecosystem, with ${numbers.vip} VIP members who are probably your biggest fans. Revenue-wise, you're sitting pretty at ¥${numbers.revenue.toLocaleString()}, and your ${numbers.campaigns} campaigns are pulling their weight with ${numbers.roi}% ROI. Customer engagement at ${numbers.engagement}% shows people actually like what you're doing - which is awesome! Want to dive deeper into any of these numbers?`
  }
...
const generateIntelligentGreetingResponse = (message: string, mode: string, data: any) => {
  ...
  const currentGreeting = greetingVariations[Math.floor(Math.random() * greetingVariations.length)]
  
  if (mode === 'serious') {
    return `${currentGreeting} I'm your AI business intelligence assistant. Your current business metrics show ${data.customerStats?.total || 'N/A'} customers generating ¥${data.dashboardStats?.totalRevenue?.toLocaleString() || '0'} with ${data.campaignStats?.avgROI || 'N/A'}% average campaign ROI. Your customer engagement is at ${data.customerStats?.avgEngagement || 'N/A'}% and you have ${data.customerStats?.segmentDetails?.length || 0} dynamic customer segments discovered by our AI. How can I assist you with data analysis, campaign optimization, or strategic insights today?`
  } else {
    ...
    const currentExcitement = excitementPhrases[Math.floor(Math.random() * excitementPhrases.length)]
    
    return `${currentGreeting} ${currentExcitement} Your SBM empire is looking absolutely spectacular - ${data.customerStats?.total || 'many'} customers, ¥${data.dashboardStats?.totalRevenue?.toLocaleString() || 'substantial revenue'} in the bank, and campaigns that are crushing it with ${data.campaignStats?.avgROI || 'impressive'}% ROI! Plus, our AI has discovered ${data.customerStats?.segmentDetails?.length || 'several'} unique customer segments that could unlock even more potential. What incredible business adventure should we embark on today? Let's create some serious magic! ✨`
  }
}
...
const analyzeMessageContext = (message: string, data: any) => {
  ...
  const contexts = {
    isAboutGrowth: lowerMessage.includes('grow') || lowerMessage.includes('expand') || lowerMessage.includes('increase'),
    isAboutCustomers: lowerMessage.includes('customer') || lowerMessage.includes('client') || lowerMessage.includes('user'),
    isAboutRevenue: lowerMessage.includes('revenue') || lowerMessage.includes('money') || lowerMessage.includes('profit'),
    isAboutStrategy: lowerMessage.includes('strategy') || lowerMessage.includes('plan') || lowerMessage.includes('approach'),
    isExploring: lowerMessage.includes('what') || lowerMessage.includes('how') || lowerMessage.includes('why'),
    isOptimistic: lowerMessage.includes('good') || lowerMessage.includes('great') || lowerMessage.includes('excellent'),
    needsHelp: lowerMessage.includes('help') || lowerMessage.includes('assist') || lowerMessage.includes('support')
  }
  
  // Generate contextual suggestions
  let suggestedAction = "monitoring current performance trends and identifying optimization opportunities"
  let engagementAnalysis = "healthy customer interaction levels"
  let creativeObservation = "the data patterns are revealing some intriguing opportunities"
  let opportunitySpotlight = "the potential for strategic growth initiatives based on current performance metrics"
  
  if (contexts.isAboutGrowth) {
    suggestedAction = "exploring expansion strategies leveraging your current customer satisfaction rates"
    opportunitySpotlight = "untapped growth potential in your existing customer segments"
  }
  
  if (contexts.isAboutCustomers) {
    suggestedAction = "implementing customer-centric optimization strategies"
    creativeObservation = "your customer engagement patterns are telling a compelling story"
    opportunitySpotlight = "hidden insights within your customer behavior data"
  }
  
  if (contexts.isAboutRevenue) {
    suggestedAction = "focusing on revenue optimization and ROI enhancement strategies"
    opportunitySpotlight = "significant revenue upside potential based on current performance indicators"
  }
...
const generateDynamicInsights = (data: any) => {
  ...
  if (segmentCount > 5) {
    insights.push("Customer segmentation diversity is providing rich optimization opportunities")
  }
  
  const professionalInsight = insights.length > 0 
    ? insights.join(', and ') + "."
    : "Current metrics indicate stable operational performance with room for strategic enhancement."
    
  const creativeInsight = insights.length > 0
...
export const api = {
  get: async (url: string) => {
    ...
    switch (path) {
        ...
        return [
          {
            ...
            message: 'Your "Young Adult Engagement" campaign exceeded ROI targets by 23%',
          ...
          },
          {
            ...
            message: 'AI detected 156 high-value prospects in the premium segment',
      ...
      case '/segmentation/overview':
        const stats = mockDataStore.getCustomerStats()
        return {
          labels: ['VIP Members', 'Regular Customers', 'New Visitors', 'Inactive'],
          values: [stats.vip, stats.regular, stats.new, stats.inactive],
          segments: [
            { name: 'VIP Members', count: stats.vip, percentage: ((stats.vip / stats.total) * 100).toFixed(1) },
            { name: 'Regular Customers', count: stats.regular, percentage: ((stats.regular / stats.total) * 100).toFixed(1) },
            { name: 'New Visitors', count: stats.new, percentage: ((stats.new / stats.total) * 100).toFixed(1) },
            { name: 'Inactive', count: stats.inactive, percentage: ((stats.inactive / stats.total) * 100).toFixed(1) }
          ],
          total: stats.total
        }
      ...
      case '/campaign-intelligence/predictions':
        return {
          predictions: [
            { campaign: 'Summer Sale 2025', predicted_roi: 195, confidence: 87 },
            { campaign: 'New Customer Welcome', predicted_roi: 265, confidence: 92 },
            { campaign: 'Reactivation Campaign', predicted_roi: 125, confidence: 74 }
          ]
        }
      ...
      case '/admin/chat':
        ...
        if (data?.message?.toLowerCase().includes('test')) {
          return {
            ...
            suggestions: ['Try saying hello', 'Ask about business', 'Test more features']
          }
        }
        
        try {
          console.log('📨 Chat request received:', { message: data?.message, mode: data?.mode })
          
          if (!data || !data.message) {
            console.error('❌ Invalid chat data:', data)
            return {
              response: 'Please provide a message to chat.',
              suggestions: ['Try typing a message', 'Ask about your business', 'Say hello']
            }
          }
          ...
          return {
            response: 'I encountered a technical issue. Let me try a simple response: Hello! I\'m your AI assistant. How can I help you today?',
            suggestions: ['Ask about your customers', 'Check campaign performance', 'Analyze business metrics', 'Get help with strategy']
          }
        }
  ...
  put: async (url: string, data?: any) => {
    ...
    if (path.includes('/campaigns/') && path.endsWith('/status')) {
      const campaignId = path.split('/')[2]
      const updated = mockDataStore.updateCampaignStatus(campaignId, data.status)
      if (updated) {
        mockDataStore.addActivity({
          type: 'campaign',
...
const isOffTopicQuestion = (message: string, messageAnalysis: any) => {
  ...
  const businessKeywords = [
    'business', 'customer', 'client', 'revenue', 'profit', 'campaign', 'marketing', 
    'sales', 'analytics', 'data', 'crm', 'segment', 'roi', 'conversion', 'engagement',
    'growth', 'optimization', 'strategy', 'performance', 'dashboard', 'metrics'
  ]
  ...
  const generalTopics = [
    'weather', 'food', 'movie', 'music', 'sport', 'game', 'book', 'travel', 'health',
    'science', 'technology', 'news', 'politics', 'entertainment', 'personal', 'family',
    'hobby', 'recipe', 'joke', 'story', 'philosophy', 'history', 'art', 'culture'
  ]
  
  const hasGeneralTopics = generalTopics.some(topic => lowerMessage.includes(topic))
  
  // Personal questions
  const personalQuestions = [
    'who are you', 'what are you', 'tell me about yourself', 'your name', 'your age',
    'where are you from', 'how old are you', 'what do you like', 'favorite'
  ]
  
  const isPersonalQuestion = personalQuestions.some(pattern => lowerMessage.includes(pattern))
  ...
  const isFactualQuestion = factualPatterns.some(pattern => pattern.test(lowerMessage))
  
  // Return true if it's clearly off-topic
  return (hasGeneralTopics || isPersonalQuestion || isFactualQuestion) && !hasBusinessKeywords
}

// 🧠 INTELLIGENT GENERAL RESPONSE GENERATOR
const generateIntelligentGeneralResponse = (message: string, mode: string, businessContext: any, timeContext: any, messageAnalysis: any) => {
  const lowerMessage = message.toLowerCase()
  ...
  // Mathematical or factual questions
    ...
    if (mathMatch) {
      ...
      const operator = lowerMessage.includes('+') ? '+' : lowerMessage.includes('-') ? '-' : lowerMessage.includes('*') ? '*' : '/'
      let result = operator === '+' ? num1 + num2 : operator === '-' ? num1 - num2 : operator === '*' ? num1 * num2 : num1 / num2
      ...
      response = mode === 'serious'
        ...
        : `That's ${result}! 🎯 You know what else I love calculating? Your business metrics! With your current ¥${businessContext.revenue.toLocaleString()} revenue and ${businessContext.roi}% ROI, there are some fascinating calculations we could explore together!`
    }
  }
  ...
  else if (lowerMessage.includes('movie') || lowerMessage.includes('music') || lowerMessage.includes('book')) {
    const topic = lowerMessage.includes('movie') ? 'movies' : lowerMessage.includes('music') ? 'music' : 'books'
    response = mode === 'serious'
  ...
  const businessTransition = mode === 'serious'
  ...
  return {
    ...
    insights: [`Conversation context: ${timeContext.greeting}`, `User engagement level: High`],
    recommendations: [
      { action: 'Continue conversation', priority: 'Medium', impact: 'Relationship building' },
      { action: 'Transition to business topics', priority: 'Low', impact: 'Strategic alignment' }
    ]
  }
}
